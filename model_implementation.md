...
`mxiter` is the maxium number of iterations
...
```fortran
      ...
      DATA NPAR,NCNSTR,ASYCV,MXITER/{{ npar }},0,.TRUE.,{{ mxiter|default(500) }}/
...
call mendel with the parameters, mendel will call the subroutines defined later
...
initialize, create arrays with initial values
...
`popbr` is the population frequencies of breast cancer.
...
`par`: an array of par (will change)
`parmin`: an array of lower bound for `par`
`parmax`: an array of upper bound for `par`
...
Fill `popbr` with initial frequencies
...
`output` function, does not really matter
...
Adjustment of likelihood, which is empty,
...
`apen` is the penetrance function
...
* `rrbr`: relative risk for breast cancer
* `cumbrrisk`: cummulative relative risk
* `popbr`: population risk for breask cancer (user provided)
* `ffbr`: survival probability carrier
* `ffncbr`: survival probability non-carrier
* `lambda`: `lambda[0]` and `labmda[1]`
* `idis`: affection status, 1 for affected
* `isex`: sex
...
c---------------------------------------------------------------------
```
input data, which is the age of disgnosis for `bc`, `bc2`, `oc`, `other`, `other2`, `lfu`, and `death`.

**NOTE**: `agebc2` and `ageother2` are not used at all.
...
if `ageother`, `agebc`, `ageoc` is `999` (`agebc2` and `ageother2` are not handled), then take it to be the minimum of `agelfu` and `agedeath` (if exists).
...
Censor at the first cancer, so

$age = \min(agebc, ageoc, ageother)$ if one of them is specified.

$age = \min(agelfu, agedeath)$ if one of them is specified.
...
If $age > 90$, assume $idis=0$ and $age=90$.
...
```
calcuate $p_1=(1-0.0004)^2$ and $p_2=1-p_1$,
...
Not sure how $male$ is passed, assuming it is from pedigree.

For each genotype
* (1,1): is=1 non-carrier
* (others): is=2, carrier
...
if breast cancer, male penetrance is always 1.
...
Prior probabilities for each founder
...
Transmission probabilities for each parent-offspring pair in a pedigree.
