...
C
      ...
      WRITE(*,'(/A)') LINE
C     Modified for non-interactive execution
C     Automatically set to non-batch mode
      BATCH=.FALSE.
      ANSWER='N'
      WRITE(*,20)
 20   FORMAT(///////
     :,'   Running in non-interactive mode (batch mode: NO)')
      END
...
C     Modified to automatically select item 21 (ACCEPT PROBLEM)
C
      ITEM=21
      WRITE(*,*) 'Auto-selecting item 21: ACCEPT PROBLEM'
...
C     Modified to automatically answer 'NO'
C
 50   ANSWER='N'
      WRITE(*,*) 'Auto-selecting: NO MORE PROBLEMS'
...
C
 100  IF (IERROR.EQ.0) GO TO 90
      ...
      WRITE(UNIT3,101) IDFAM, PERSON(INODE)
101   FORMAT(/, 'IERROR IGNORED on ', A8, ' near ', I4)
      GO TO 90
      END IF
      IF (PMODE) INODE=(INODE-1)/NLOCI+1
